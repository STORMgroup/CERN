...
"$CERN_PATH" "$MODEL" "${DATA_DIR}/d1_ecoli_training_${SEGMENTER}_events.tsv" "$PSTAY" "$PSKIP" --window-size "${WINDOWSIZE}" > "${DATA_DIR}/d1_ecoli_training_${SEGMENTER}_events_corrected.tsv"
...
OUTDIR="out_rh2"
EVENTS="${DATA_DIR}/d1_ecoli_training_${SEGMENTER}_events_corrected.tsv"
PARAMS="--chunk-size 99999999 --r10 --sig-diff -1 --events-file ${EVENTS}"
